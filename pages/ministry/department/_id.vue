<template>
  <div>
    ...
    <h1 class="my-2 text-h4">ກົມຈັດຕັ້ງທັງໝົດ ( {{ departmentDO?.count }} )</h1>
    <v-data-table
        ...
        <div class="d-flex">
          <div>
            <v-btn
              ...
              color="teal"
              dark
              @click="createPhane(item.id)">ສ້າງພະແນກ</v-btn>
          </div>
        </div>
      </template>
      <template v-slot:expanded-item="{ headers, item }">
        <td :colspan="headers.length" :ref="item.id">
          <v-col v-for="data in getDepartment(item.id)" :key="data.id" cols="12">
            <v-card elevation="0">
...
export default {
  ...
  methods: {
    ...
    getDepartment(id) {
      ...
      return this.department.rows.filter(el => el.department_organization_id === id)
    }
  },
};
</script>
