<template>
    <div>
      <h1 class="my-10">ຈັດການຂໍ້ມູນ admin ຂອງທ້ອງຖີ່ນ (<span class="primary--text"> {{ department?.count }} ຄົນ </span> ) </h1>
      <v-row>
        <v-col md="6">
          <v-text-field
            v-model="search"
  ...
  export default {
    ...
    methods: {
      ...
      try {
        var list = [],
          index = 0;
        for (let i = 0; i < this.department?.rows?.length; i++) {
          var el = this.department?.rows[i];
          index = parseInt(i) + 1;
          var obj = {
            idx: index,
            name: el.user_name,
